            ...
            <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
                <div className="flex flex-col gap-4 flex-1">
                    ...
                    <div className="flex flex-wrap gap-6">
                        ...
                        <div className="flex items-center gap-2">
                            <input type="number" id='regularPrice' min='70000' required className='p-3 border border-gray-300 rounded-lg w-48' onChange={handleChange} value={formData.regularPrice}/>
                        ...
                        </div>
                        {
                            ...
                            <input type="number" id='discountPrice' required className='p-3 border border-gray-300 rounded-lg w-48' onChange={handleChange} value={formData.discountPrice} />
                            <div className="flex  flex-col text-center">
                ...
                <div className="flex flex-col flex-1 gap-4">
                    <p className='font-normal text-gray-600 ml-2'>Images: <span>The first image will be the cover (max 6)</span></p>
                ...
                <p className='text-red-700 text-sm text-center font-semibold'>{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div key={url} className="flex justify-between p-3 border items-center">
                            <img src={url} alt="" className='w-20 h-20 object-contain rounded-lg' />
                            <button type='button' onClick={()=>handleRemoveImge(index)} className='text-red-700 rounded-lg uppercase hover:opacity-75'>Delete</button>
                        </div>
                    ))
                }

                    <button disabled={loading || uploading} className="p-3 bg-[#1D24CA] text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80">{loading ? 'Creating...': 'Create Listing'}</button>
